...
Prefer the discovery-first CLI so Hermes validates the server and writes config safely:
...
If scripting a server with many discovered tools, the add command may prompt `Enable all N tools?`; pipe `y` if you intentionally want all of them:

```bash
printf 'y\n' | hermes mcp add gemini-notebook-mcp --command {USER_HOME}/.local/bin/notebooklm-mcp.exe --connect-timeout 90
```

Manual config equivalent (use only when the CLI cannot express what you need):

...
When the user types `/opensquad` or asks for the menu, present an interactive selector using Codex `request_user_input` when available. If it is unavailable, ask one concise plain-text question and wait for the user's reply.
...
If the user selects "More options", present a second selector using the same Codex-compatible question behavior:
...
- If an Opensquad instruction mentions `AskUserQuestion`, use Codex `request_user_input` when that tool is available in the current mode.
- If `request_user_input` is unavailable, ask one concise plain-text question and wait for the user's reply.
...
- Keep all user-facing Opensquad responses in the language configured in `_opensquad/_memory/preferences.md`.
